...
2. Получает или создает сессию для ID чата
3. Если нет текущего состояния, входит в состояние по умолчанию
4. Обработчик onMessage текущего состояния обрабатывает сообщение
5. В зависимости от возвращаемого значения:
...
Смотрите [`examples/tickets.ts`](./examples/tickets.ts) для полного примера системы тикетов поддержки, демонстрирующего
управление состояниями, обработку файлов и сложные потоки диалогов.

### Реализация пользовательского хранилища

Проверьте [`examples/custom-storage/`](./examples/custom-storage/custom-storage-example.ts) для примера реализации пользовательского провайдера
